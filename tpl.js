...
function refresh(node, datas) {
    ...
    _make(node, datas)
    if (window.fixSvg) window.fixSvg(node)
}

function make(targetNode, datas) {
    _make([targetNode], datas)
    if (window.fixSvg) window.fixSvg(targetNode)
}

module.exports = {refresh, make}
